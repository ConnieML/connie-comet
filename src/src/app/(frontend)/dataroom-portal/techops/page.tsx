...
export default function TechOpsDataroomPage() {
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      const categories = ['architecture', 'security', 'infrastructure', 'apis']
  ...
  const categories = [
    {
      ...
      count: categoryCounts.architecture || 0,
      icon: '🏢',
      description: 'System design, architecture diagrams, technical specifications'
    },
    {
      category: 'security',
      ...
      icon: '🔒',
      description: 'Security policies, compliance docs, audit reports, security procedures'
    },
    {
      category: 'infrastructure',
      label: 'Infrastructure & DevOps',
      count: categoryCounts.infrastructure || 0,
      icon: '☁️',
      description: 'Deployment guides, infrastructure docs, DevOps procedures'
    },
    {
      category: 'apis',
      label: 'API Documentation',
      count: categoryCounts.apis || 0,
      icon: '🔌',
      description: 'API specs, integration guides, developer documentation'
    }
  ]
      ...
      <div className="container mx-auto px-6 py-16 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-600 max-w-2xl mx-auto mb-4">
            Architecture, security, infrastructure, and API documentation
          </p>
          ...
          {authLoading ? (
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <a
                href="/api/auth/okta"
                className="inline-flex items-center px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-full transition-colors"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="max-w-4xl mx-auto">
            ...
            {categories.map((category) => {
              ...
              const content = (
                <div className={`backdrop-blur-sm rounded-2xl p-8 transition-all duration-300 shadow-lg bg-white/60 border border-slate-200 ${
                  isClickable 
                    ? 'hover:transform hover:scale-[1.02] hover:shadow-xl hover:bg-white/80 hover:border-green-500/50' 
                    : 'opacity-75'
                }`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className={`text-3xl font-light mb-1 ${isAuthenticated ? 'text-green-600' : 'text-slate-400'}`}>
                        {category.count}
                      </div>
              ...
              if (isClickable) {
                return (
                  <Link
                    ...
                    href={`/dataroom-portal/techops/${category.category}`}
                    className="group"
                  >
                    {content}
                  </Link>
          ...
          <div className="text-center mt-12">
            ...
            <div className="mt-4">
              <span className="inline-block px-3 py-1 bg-green-100 text-green-700 text-xs rounded-full border border-green-300">
                TECHNICAL OPERATIONS • Secure Access
              </span>
            </div>
          </div>
        </div>
